...
**Theta-Beta Neurofeedback Anchor** → high `tbr` (>1.5), low `focus`, high `adhd_index`
...
**Sleep Onset Wind-Down** → high `drowsiness` post-session, `insomnia_index` > 50
...
**Wake Reset / Alertness Boost** → `narcolepsy_index` > 40, daytime drowsiness, `wakefulness` < 30
...
**Progressive Muscle Relaxation** → high `insomnia_index`, physical tension, high beta hyperarousal
...
**Grounding (5-4-3-2-1)** → anxiety, panic onset, high `anxiety_index`, dissociation
...
**Wim Hof Breathwork** → near-zero `engagement`, very low `wakefulness`, full reset needed
  ...
  ⚠ Not for `epilepsy_risk` > 30 or cardiovascular concerns.
...
**Buteyko CO2 Retraining** → chronic anxiety with habitual over-breathing, high `lf_hf_ratio`
...
**Power Nap Guidance** → `wakefulness` < 30, `narcolepsy_index` > 40, very high `drowsiness`
...
**Anxiety Surfing** → high `anxiety_index` + urge to escape or avoid
  Ride the wave rather than fight it. Map anxiety in the body. Breathe into the centre.
  Watch it peak and subside without acting on it. ~8 min.

**Fear Processing** → high `anxiety_index` + freeze pattern (very low `engagement`)
...
**Cervical Decompression** → chronic neck compression, forward-head posture after desk work
  Chin tuck series, head nod (yes/no), diagonal stretch, towel traction cue. ~8 min.
...
**Full Eye Exercise Sequence** → eye fatigue, visual tension, >90 min screen time
  Palming (30 s) → near-far focus shifts (10 reps) → figure-8 tracking → lateral saccades
...
*(The LLM cannot play music, but gives precise, actionable cues — genre, BPM range,
specific artists/playlists, and how to listen. Always offer a specific recommendation.)*
...
**Binaural Beat Entrainment** → target a specific frequency state before cognitive work
  Alpha (8–12 Hz): relaxed focus / creative work.
  Theta (4–8 Hz): deep meditation, integration, hypnagogic.
  Gamma (30–50 Hz): analytical intensity, integration.
  Requires stereo headphones. ~20–30 min eyes closed or soft-gaze.

**Music-Breath Synchronisation** → cardiac coherence variant using BPM as pacer
  Choose a track ~60 BPM. Inhale one bar, exhale the next. Easier than counted breathing.
  Good for users who find silent breath-counting frustrating. ~5–10 min.
...
**Rhythm Grounding** → anxiety, dissociation, freeze, high `anxiety_index`
  Find a steady beat (metronome or drum track). Tap along slowly. Gradually slow the tapping.
  Rhythmic regularity down-regulates via predictive entrainment. ~5 min.
...
**Emotional Release with Music** → grief, anger, or unprocessed emotion needing discharge
  1. Choose track matching the exact emotion. 2. Allow it to amplify the feeling.
  3. When peak passes, switch to a quieter, resolving piece. 4. Sit in silence 2 min. ~20 min.
...
**FOMO Defusion** → anxiety about missing out, compulsive checking, high `anxiety_index`
...
**Digital Sunset Protocol** → `insomnia_index` elevated, pre-sleep phone use
...
**Caffeine Timing Protocol** → afternoon focus crash; `anxiety_index` elevated; coffee question
  ...
  If `anxiety_index` > 50, switch to matcha/green tea.
...
**Mood-Food Connection** → `depression_index` > 35, persistently low `mood`, low `faa`
