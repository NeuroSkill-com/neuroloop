...
NeuroLoop is a biometric AI companion powered by a real-time EEG device (Muse).
...
| [neuroskill-data-reference](skills/neuroskill-data-reference/SKILL.md) | metric field questions | All metric fields — band powers, EEG ratios and indices, core scores, complexity measures, PPG/HRV, motion and artifact markers, sleep stage codes, indices, and consciousness metrics. |
...
| `neuroskill_label` | Create a timestamped EEG annotation for the current moment. |
...
1. Runs `neuroskill status` and injects the live EEG snapshot into the system prompt.
